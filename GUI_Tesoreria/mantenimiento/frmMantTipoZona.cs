...
{
    public partial class frmMantTipoZona : frmPadre
    {
        ...
        private static frmMantTipoZona frmInstance = null;

        public frmMantTipoZona()
        {
            InitializeComponent();
        }

        public static frmMantTipoZona Instance()
        {
            if (((frmInstance == null) || (frmInstance.IsDisposed == true)))
            {
                frmInstance = new frmMantTipoZona();
            }
            ...
            if (this.dgvServicio.RowCount > 0)
            {
                ...
                try
                {
                    if (accion == "D")
                    {
                        ...
                        {
                            if (cd.EjecutarSP("usp_mantenimiento_tipo_zona", dgvServicio.Rows[indice].Cells[0].Value, "",
                                "", accion) == 1)
                            {
            ...
            try
            {
                ...
                if (accion == "I")
                {
                    if ((DevComponents.DotNetBar.MessageBoxEx.Show("¿Seguro de ingresar sucursal?", VariablesMetodosEstaticos.encabezado,
                                MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes))
                    {
                        cd.EjecutarSP("usp_mantenimiento_tipo_zona", 0, txtNombre.Text.Trim(), txtDescripcion.Text.Trim(), accion);
                ...
                {
                    cd.EjecutarSP("usp_mantenimiento_tipo_zona", dgvServicio.Rows[indice].Cells[0].Value, txtNombre.Text.Trim(), txtDescripcion.Text.Trim(), accion);
        ...
        private void frmMantTipoZona_Load(object sender, EventArgs e)
        {
        ...
        public void CargaDatosxId()
        {
            try
            {
                ...
                if (dgvServicio.RowCount > 0)
                {
                    ...
                    dt = cd.TraerDataset("usp_select_tipo_zona_all_x_id", xcod).Tables[0];
        ...
        private void cargarServicios()
        {
            ...
            try
            {
                dt = cd.TraerDataset("usp_select_tipo_zona_all_x_id", 0).Tables[0];
                dgvServicio.Refresh();
