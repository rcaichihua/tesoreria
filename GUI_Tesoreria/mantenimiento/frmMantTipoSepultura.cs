...
{
    public partial class frmMantTipoSepultura : frmPadre
    {
        private int xcod;
        ...
        private static frmMantTipoSepultura frmInstance = null;

        public frmMantTipoSepultura()
        {
            InitializeComponent();
        }

        public static frmMantTipoSepultura Instance()
        {
            if (((frmInstance == null) || (frmInstance.IsDisposed == true)))
            {
                frmInstance = new frmMantTipoSepultura();
            }
            frmInstance.BringToFront();
            return frmInstance;
        }

        private void frmMantTipoSepultura_Load(object sender, EventArgs e)
        {
            //this.BackgroundImage = Image.FromFile(varglo.RUTA_FONDO + "FONDOCON.png");
            cargarTipoSepultura();
            if (dgvTipSepultura.RowCount > 0) { CargaDatosxId(); }
        ...
        {
            if (this.dgvTipSepultura.RowCount > 0)
            {
                int indice = dgvTipSepultura.CurrentRow.Index;
                accion = "D";
                try
                {
                    if (accion == "D")
                    {
                            ...
                            MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes))
                        {
                            ...
                            ta_tipo_sepultura tipSep = new ta_tipo_sepultura();

                            tipSep.intTsId= Convert.ToInt32(txtId.Text.Trim());

                            cn.eliminarTipoSepultura(tipSep);
        ...
        private void btnCancelar_Click(object sender, EventArgs e)
        {
            ...
            cargarTipoSepultura();
            accion = "";
            dgvTipoSepultura_Click(sender, e);
        }
...

            {
                ...
                if (accion == "I")
                {
                                ...
                                MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes))
                    {
                        ...
                        tipSep.varTsDescripcion= txtNombre.Text.Trim();
                        tipSep.varTsObservacion = txtDescripcion.Text.Trim();

                        rspta=cn.insertarActualizarTipoSepultura(tipSep);
        ...
        private void dgvTipoSepultura_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            dgvTipoSepultura_Click(sender, e);
        }

        private void dgvTipoSepultura_Click(object sender, EventArgs e)
        {
            if (dgvTipSepultura.RowCount > 0) { CargaDatosxId(); }
        }
        ...
        public void CargaDatosxId()
        {
            ...
            {
                int fila;
                if (dgvTipSepultura.RowCount > 0)
                {
                    fila = dgvTipSepultura.CurrentRow.Index;
                    xcod = Convert.ToInt16(dgvTipSepultura.Rows[fila].Cells[0].Value);

                    this.txtId.Text = (dgvTipSepultura.Rows[fila].Cells[0].Value.ToString());
                    this.txtNombre.Text = (dgvTipSepultura.Rows[fila].Cells[1].Value.ToString());
                    this.txtDescripcion.Text = (dgvTipSepultura.Rows[fila].Cells[2].Value.ToString());
                }
            }
            catch (Exception)
            {
        ...
        private void cargarTipoSepultura()
        {
            ...
            try
            {
                ...
                dt=cn.listarTipoSepultura();
                dgvTipSepultura.Refresh();
                dgvTipSepultura.DataSource = dt;
            }
            catch (Exception ex)
            {
