...
namespace GUI_Tesoreria
{
    ...
    {
        private const string ExcelOleDbConnectionStringTemplate = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0};Extended Properties=\"Excel 12.0;HDR=YES\";";
            ...
            try
            {
                ...
                using (var conn = new OleDbConnection(String.Format(ExcelOleDbConnectionStringTemplate, fullFilePath)))
                {
                    if (conn.State != ConnectionState.Open)
                    {
                        ...
                        string tableToDelete = "Table";   //table name
                        ...
                        if (tableExists)
                        {
                            using (OleDbCommand cmd1 = new OleDbCommand(string.Format("DROP TABLE {0}", "[" + tableToDelete + "]"), conn))
                            {
                                cmd1.ExecuteNonQuery();
                                //message table deleted correct
                            }
                        }
                        //else
